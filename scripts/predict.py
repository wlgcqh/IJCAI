...
#生成提交结果
def Submission():
    koubei_test_log = open(parent_path+'/data/datasets/seg_data/ijcai2016_koubei_test_seg_sub').readlines()
    Dict = location_merchant()

    submission = open(parent_path+'/data/datasets/seg_data/submission.csv','w')
    submission_log = csv.writer(submission)

    hotMerchant_dict = hotMerchant_stat(parent_path+'/data/datasets/seg_data/ijcai2016_koubei_train_seg')
    hotLocationMerchant_dict = hotLocationMerchant_stat(parent_path+'/data/datasets/seg_data/ijcai2016_koubei_train_seg')
    train_dict = train_data()
    #taobao_user_dict = readPersistence('user_dict.txt')
    merchant_dict = merchant_feature()
    index = 0

    #trainData = np.load('user_feature.npy')
    print 'Normalized!'
    ...
    for line in koubei_test_log:
        ...
        #print user_id,location_id
        merchant_ids = Dict[location_id]
        hotLocationDict = hotLocationMerchant_dict.get(location_id,None)
        _dict = dict()
        ...
        try:
            ...
            #if taobao_user_dict.has_key(user_id):
            #   recommandMerchants = dict(sorted(_dict.items(),key = lambda x :x[1],reverse = True)[:5]).keys()
            #   taobao+=1
            #else:
            recommandMerchants = dict(sorted(_dict.items(),key = lambda x :x[1],reverse = True)[:1]).keys()
    ...
    print train,taobao,notaobao,index
